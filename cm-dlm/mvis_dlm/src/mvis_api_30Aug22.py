...
TrainProcessedInfo.v4_fft_max_freq, TrainProcessedInfo.v4_fft_sd, TrainProcessedInfo.rake_id]).execute()
    ...
    def insert_mvis_processed_info(self, data):
        ...
        try:
            ...
            for idx in range(self.json_data["total_bogie"]):
            ...
            for idx in range(self.json_data["total_wagon"]):
                wagon_id = "W" + str(idx+1)
                new_tagged_wagon_list.append(wagon_id)
                new_tagged_wagon_list.append(wagon_id)
                #actual_wagon_id_list.append(self.json_data["wagon_id"][idx])
                #actual_wagon_id_list.append(self.json_data["wagon_id"][idx])
                #actual_wagon_type_code_list.append(self.json_data["wagon_type"][idx])
                #actual_wagon_type_code_list.append(self.json_data["wagon_type"][idx])
                new_defect_wagon_id_list.append("none")
                new_defect_wagon_id_list.append("none")

            for bogie_idx in range(len(new_tagged_bogie_list)):
                for defect_idx in range(len(self.json_data["defects"])):
                    if self.json_data["defects"][defect_idx]["defect_bogie_id"] == new_tagged_bogie_list[bogie_idx]:
                        Log.logger.info(f'bogie id matched - {self.json_data["defects"][defect_idx]["defect_bogie_id"]}, {new_tagged_bogie_list[bogie_idx]}')
                        new_defect_bogie_id_list[bogie_idx] = self.json_data["defects"][defect_idx]["defect_bogie_id"]
                        new_defect_code_list[bogie_idx] = self.json_data["defects"][defect_idx]["defect_code"]
                        new_defect_image_list[bogie_idx] = self.json_data["defects"][defect_idx]["defect_image"]
                        new_defect_side_list[bogie_idx] = self.json_data["defects"][defect_idx]["side"]
                    else:
                        pass 
            
            for wagon_idx in range(len(new_tagged_wagon_list)):
                for defect_idx in range(len(self.json_data["defects"])):
                    if self.json_data["defects"][defect_idx]["defect_wagon_id"] == new_tagged_wagon_list[wagon_idx]:
                        Log.logger.info(f'wagon id matched - {self.json_data["defects"][defect_idx]["defect_wagon_id"]}, {new_tagged_wagon_list[wagon_idx]}')
                        if new_defect_bogie_id_list[wagon_idx] != '-':
                            new_defect_wagon_id_list[wagon_idx] = self.json_data["defects"][defect_idx]["defect_wagon_id"]
                        else:
                            pass
                    else:
                        pass

            #for wagon_type_idx in range(len(actual_wagon_type_code_list)):
                #wagon_type_code = actual_wagon_type_code_list[wagon_type_idx] 
            ...
            #Log.logger.info(f'New actual wagon id list: {actual_wagon_id_list}')
            #Log.logger.info(f'New actual wagon type code list: {actual_wagon_type_code_list}')
            #Log.logger.info(f'New actual wagon type list: {actual_wagon_type_list}')
