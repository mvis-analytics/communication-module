...
client.connect("157.245.101.18", 1883, 60)
...
while True:
    ...
    dpu_id = "DPU_01"
    ts = time.time()
    train_processed_info = {"ts": ts, "train_id" : train_id, "dpu_id": dpu_id, "axle_id": axle_id, "axle_speed": axle_speed, "avg_dyn_load_left": avg_dyn_load_left, "avg_dyn_load_right": avg_dyn_load_right, "max_dyn_load_left": max_dyn_load_left, "max_dyn_load_right" : max_dyn_load_right, "vertical_load_left" : vertical_load_left, "vertical_load_right": vertical_load_right, "lateral_load_left" : lateral_load_left,"lateral_load_right": lateral_load_right, "ilf_left": ilf_left, "ilf_right": ilf_right, "wheel_status_left" : wheel_status_left, "wheel_status_right" : wheel_status_right, "v1_fft_max_freq": v1_fft_max_freq, "v2_fft_max_freq": v2_fft_max_freq, "v3_fft_max_freq": v3_fft_max_freq, "v4_fft_max_freq": v4_fft_max_freq, "v1_fft_sd": v1_fft_sd, "v2_fft_sd": v2_fft_sd, "v3_fft_sd": v3_fft_sd, "v4_fft_sd": v4_fft_sd, "rake_id": rake_id, "left_def_info_id": left_def_info_id, "right_def_info_id": right_def_info_id}
    json_train_processed_info = json.dumps(train_processed_info)
   
    train_exit_time =time.time()
    
    #train_consolidated_info = {"train_id" : train_id, "dpu_id" : dpu_id, "train_entry_time": train_entry_time, "train_exit_time": train_exit_time, "total_axles": total_axles,"total_wheels": total_wheels, "total_bad_wheels": total_bad_wheels, "direction": direction, "train_speed": train_speed, "ilf_threshold_warning": ilf_threshold_warning, "ilf_threshold_critical": ilf_threshold_critical, "mdil_threshold_warning": mdil_threshold_warning, "mdil_threshold_critical": mdil_threshold_critical, "train_type": train_type, "train_processed": train_processed, "remark": remark, "train_no_name": train_no_name, "engine_code": engine_code, "engine_no": engine_no, "zone": zone, "division": division, "depot": depot}
    train_consolidated_info = {"train_id" : train_id, "dpu_id" : dpu_id, "train_entry_time": train_entry_time, "train_exit_time": train_exit_time, "total_axles": total_axles,"total_wheels": total_wheels, "total_bad_wheels": total_bad_wheels, "direction": direction, "train_speed": train_speed, "ilf_threshold_warning": ilf_threshold_warning, "ilf_threshold_critical": ilf_threshold_critical, "mdil_threshold_warning": mdil_threshold_warning, "mdil_threshold_critical": mdil_threshold_critical, "train_type": train_type, "train_processed": train_processed, "remark": remark}

    json_train_consolidated_info = json.dumps(train_consolidated_info)

    client.publish("site_01/dpu_01/train_consolidated_info", json_train_consolidated_info)
    time.sleep(0.05)
    #client.publish("site_01/dpu_01/train_processed_info", json_train_processed_info)
